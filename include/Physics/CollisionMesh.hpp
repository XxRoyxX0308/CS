#ifndef CS_PHYSICS_COLLISION_MESH_HPP
#define CS_PHYSICS_COLLISION_MESH_HPP

#include "Physics/CollisionTypes.hpp"
#include "Core3D/Model.hpp"

#include <glm/glm.hpp>
#include <vector>
#include <cstdint>

namespace Physics {

/**
 * @brief Axis-aligned bounding box on the XZ plane.
 * 
 * Used for broadphase spatial queries against the collision grid.
 */
struct AABB_XZ {
    float minX, maxX;
    float minZ, maxZ;
};

/**
 * @brief Mesh-based collision data with spatial acceleration structure.
 *
 * Extracts all non-degenerate triangles from a loaded 3D model, transforms
 * them into world space, precomputes normals, and bins them into a cache-
 * friendly packed uniform grid for fast broadphase queries.
 */
class CollisionMesh {
public:
    CollisionMesh() = default;

    /**
     * @brief Build the collision mesh and spatial grid from a model.
     * 
     * Extracts triangles from the model, transforms them to world space,
     * and builds a 2D spatial grid for efficient broadphase queries.
     * 
     * @param model     The loaded 3D model (e.g., map geometry).
     * @param transform World-space transform (rotation + scale).
     */
    void Build(const Core3D::Model &model, const glm::mat4 &transform);

    /**
     * @brief Gather candidate triangle indices overlapping an XZ AABB.
     *
     * Returned indices may contain duplicates (a triangle can span
     * multiple cells). The caller is responsible for deduplication
     * if needed (typically via sorting + unique).
     *
     * @param aabb Query region on the XZ plane.
     * @param out  Output vector - candidate indices are appended.
     */
    void GatherCandidates(const AABB_XZ &aabb,
                          std::vector<uint32_t> &out) const;

    // ── Accessors ─────────────────────────────────────────────────────────

    /** @brief Get all triangles in world space. */
    const std::vector<Triangle> &GetTriangles() const { return m_Triangles; }
    
    /** @brief Get the number of triangles in the mesh. */
    size_t GetTriangleCount() const { return m_Triangles.size(); }
    
    /** @brief Check if the collision mesh has been built. */
    bool IsBuilt() const { return m_Built; }

private:
    /**
     * @brief Convert world XZ coordinates to flat grid cell index.
     * @return Cell index, or -1 if out of bounds.
     */
    int GridIndex(float x, float z) const;

    // ── Triangle Storage ──────────────────────────────────────────────────
    std::vector<Triangle> m_Triangles;  ///< All triangles in world space

    // ── Spatial Grid ──────────────────────────────────────────────────────
    // Packed flat grid: m_CellOffsets[i]..m_CellOffsets[i+1] index into
    // m_TriIndices for the triangle indices in cell i.
    std::vector<uint32_t> m_CellOffsets;  ///< Prefix-sum offsets (size = numCells + 1)
    std::vector<uint32_t> m_TriIndices;   ///< Packed triangle indices

    int m_GridResX = 0;
    int m_GridResZ = 0;
    float m_MinX = 0.0f, m_MinZ = 0.0f;
    float m_MaxX = 0.0f, m_MaxZ = 0.0f;
    float m_CellSizeX = 1.0f;
    float m_CellSizeZ = 1.0f;

    bool m_Built = false;
};

} // namespace Physics

#endif // CS_PHYSICS_COLLISION_MESH_HPP
